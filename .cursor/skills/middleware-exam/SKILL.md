...
**典型架构 — 缓存旁路（Cache Aside）**

```
client → request → Nginx/PHP（应用层）
                        ↓
              Redis 里有数据？ ──是──→ 直接返回（命中）
                        │
                       否（未命中）
                        ↓
                     MySQL（回源查库）
...
| 场景 | 路径 |
|------|------|
| **Redis 里有数据** | 应用从 **Redis** 读取，不访问 MySQL |
| **Redis 里没有数据** | 应用查 **MySQL**，通常再写入 Redis |

**易考点**

- Redis = **内存 KV** + 可选**持久化** + **网络**
- 图中角色：**缓存层**（Green），MySQL 为**持久化存储**
- 与 Nginx/PHP 应用配合，减轻数据库压力

#### Redis 特性（八大优势）

1. **速度快**
2. **基于键值对的数据结构服务器**（不仅是简单 KV，还支持多种数据结构）
3. **丰富的功能**
4. **简单稳定**
5. **客户端语言多**（多语言 SDK 支持）
6. **持久化**
...
> 快、结构、功能、稳、多语言、持久化、主从、高可用分布式

**与 Memcached 对比预告（考试常考）**

| Redis 独有/更强 | Memcached 通常 |
|-----------------|----------------|
| 持久化、主从、高可用 | 纯内存缓存，更简单 |
| 丰富数据结构 | 简单 KV |
| 分布式能力 | 客户端分片为主 |
...
| 场景 | 说明 |
|------|------|
| **缓存** | 热点数据加速，减轻 DB 压力（见 Cache Aside） |
| **排行榜** | 利用有序集合（Sorted Set）等结构 |
| **计数器应用** | 原子递增（浏览量、点赞数等） |
| **社交网络** | 关系、关注、动态流等 |
| **消息队列** | List / Pub-Sub 等实现轻量消息 |

**不适合场景 ❌**

| 场景 | 原因 |
|------|------|
| **大规模数据** | 内存成本高；数据量过大超出内存或性价比低 |
| **冷数据** | 访问频率低，放内存不划算，宜用磁盘存储 |

**易考点**

- Redis 适合 **热数据、高并发、结构化 KV 场景**
- 不适合 **海量冷数据** — 内存贵、容量有限
...
| 参数名 | 含义 |
|--------|------|
| **daemonize** | 默认 **不在后台**运行；需后台运行改为 **yes** |
| **pidfile** | PID 文件路径，默认 **`/var/run/redis.pid`**；多实例需不同 pidfile 和 port |
| **bind** | 只接收指定 **IP** 的请求；不设置则处理**所有**请求 |
| **port** | 监听端口，默认 **6379** |
| **timeout** | 客户端连接**超时**（秒）；超时无指令则**关闭连接** |
| **loglevel** | 日志级别 4 级：**debug / verbose / notice / warning**；一般开 **notice** |
| **logfile** | 日志文件路径；默认**标准输出**（打印到终端） |
| **databases** | 数据库个数；`SELECT <dbid>` 切换；默认使用 **db 0** |
...
- 通过在内存中**缓存数据**，减少向数据库的**频繁访问/连接次数**
- 提高**动态、数据库驱动**类网站的**运行速度**
...
> 本章目录：**Kafka 简介** ★、**Kafka 基础操作** ★（★ = 课件标注重点）
>
...
| 组件/项目 | Kafka 版本 | 节点数 | 数据流向 |
|-----------|------------|--------|----------|
| **BC-ETL 套件** | 0.10.1.1 | 3 | Yafe → **kafka** → BC-ETL |
| **内蒙古大数据平台** | 0.8.2.2 | 16 | 中创 → sdtp 解封装 → **kafka** → flume → hdfs → hive |
| **河北大数据平台** | 0.10.1.1 | 40 | 应用采集 → flume → **kafka** |
| **上海统一汇聚平台** | 0.10.1.1 | 50 | 采集应用 → SDTP → **kafka** → flume → **kafka**；**Kafka** → storm → redis |
| **集中化经分位置基地** | 0.10.1.1 | 184 | 上报网关 → flume → **kafka** |
| **集中化经分三期** | 2.0.0 | 480 | 上报网关 → flume → **kafka** |

**规律总结**

| 维度 | 课件要点 |
|------|----------|
| **版本** | 0.8.2.2 → 0.10.1.1 → **2.0.0**（随项目演进） |
| **规模** | **3 节点**（小）～ **480 节点**（大规模经分三期） |
| **常见上游** | 采集应用、上报网关、Yafe、SDTP、sdtp 解封装 |
| **常见搭档** | **Flume**（采集/转发）、**Storm**（流处理）、**HDFS/Hive**（存储）、**Redis**（快速访问） |
| **Kafka 角色** | **消息缓冲/管道**：衔接采集层与下游 ETL、存储、计算 |

**易考点**

- Kafka 在项目中多作**数据管道/消息中枢**，非孤立部署
- 典型链路：**采集 → Flume → Kafka →（Storm/ETL/HDFS）**
- 节点数反映业务规模：经分三期 **480 节点** 为课件最大规模
...
| 目标 | 说明 |
|------|------|
| **高吞吐率** | 廉价商用机器上，**单机**可支持每秒 **100 万条**消息读写 |
| **消息持久化** | 所有消息**持久化到磁盘**；**无消息丢失**；支持**消息重放** |
| **完全分布式** | **Producer、Broker、Consumer** 均支持**水平扩展** |

**总结（课件）**

> 同时满足适应**在线流处理**和**离线批处理**。

**易考点**

| 对比项 | Kafka | Redis 作消息队列 |
|--------|-------|------------------|
| 吞吐 | **百万级/秒/单机** | 较低 |
| 持久化 | **磁盘持久化 + 重放** | 可选，非核心 |
